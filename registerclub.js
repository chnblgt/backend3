app.post('/registerClub', (req, res) => {
    ...
    const query = `INSERT INTO clubs (name, category, description, email, phone, website, address, district, pricing_type, founded_year, approved) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1)`;

    db.query(query, [name, category, description, email, phone || null, website || null, address || null, district || null, pricingType || 'free', foundedYear || null], (err, result) => {
        if (err) {
